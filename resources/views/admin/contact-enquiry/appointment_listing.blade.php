            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <h5 class="text-dark font-weight-bold my-1 mr-5">
                        {{Config('constants.APPOITMENT.APPOITMENT_TITLE')}}
                    </h5>
...
<div aria-hidden="false" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="getting_basic_list_popover" class="modal fade in" style="display: none;">
    ...
    <div class="d-flex flex-column-fluid">
        <div class=" container ">
            {{ Form::open(['method' => 'get','role' => 'form','route' => "$model.appointmentListing",'class' => 'kt-form kt-form--fit mb-0','autocomplete'=>"off"]) }}
            {{ Form::hidden('display') }}
            <div class="row">
                <div class="col-12">
                                    ...
                                    <div>
                                        ...
                                        <div class="row mt-8">
                                            <div class="col-lg-12">
                                                ...
                                                &nbsp;&nbsp;
                                                <a href='{{ route("$model.appointmentListing")}}' class="btn btn-secondary btn-secondary--icon">
                                                    <span>
                                        ...
                                        <thead>
                                            <tr class="text-uppercase">
                                                <th class="{{(($sortBy == 'name' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'name' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.appointmentListing',array(	'sortBy' => 'name',
                                                   'order' => ($sortBy == 'name' && $order == 'desc') ? 'asc' : 'desc',	
                                                   $query_string))}}">Name</a>
                                                </th>

                                                <th class="{{(($sortBy == 'email' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'email' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.appointmentListing',array( 'sortBy' => 'email',
                                                ...
                                                <th class="{{(($sortBy == 'phone_number' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'phone_number' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.appointmentListing',array( 'sortBy' => 'phone_number',
                                                ...
                                                <th class="{{(($sortBy == 'message' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'message' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.appointmentListing',array( 'sortBy' => 'message',
                                                        'order' => ($sortBy == 'message' && $order == 'desc') ? 'asc' : 'desc',    
                                                        $query_string))}}">Message</a>
                                                </th>
                                                <th class="{{(($sortBy == 'created_at' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'created_at' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.appointmentListing',array(	'sortBy' => 'created_at',
                                                ...
                                                <th class="{{(($sortBy == 'is_active' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'is_active' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.appointmentListing',array(	'sortBy' => 'is_active',
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                               ...
                                               <td class="text-right pr-2">
                                                        <a href='javascript:void(0);' class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="View" onclick="getPopupClient(<?php echo $result->id; ?>)">
...
<script type="">
        ...
        $.ajax({
            url: '<?php echo URL()->to('adminpnlx/appointment/details') ?>/' + id,
            type: "POST",
            headers: {
